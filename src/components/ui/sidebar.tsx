...
>(({ asChild = false, size = "md", isActive, className, ...props }, ref) => {
  ...
  return (
    <Comp
      ...
      data-active={isActive}
      className={cn(
        "overflow-hidden1 flex h-8 min-w-0 -translate-x-px items-center gap-2 rounded-md px-2 text-muted-foreground outline-none ring-ring hover:text-foreground focus-visible:ring-2 disabled:pointer-events-none disabled:opacity-50 aria-disabled:pointer-events-none aria-disabled:opacity-50 [&>span:last-child]:truncate [&>svg]:size-4 [&>svg]:shrink-0 [&>svg]:text-accent-foreground",
        "data-[active=true]:text-primary",
        size === "sm" && "text-xs",
        size === "md" && "text-sm",
        "group-data-[collapsible=icon]:hidden",
        isActive &&
          "before:absolute before:-left-2.5 before:h-4/5 before:w-1 before:rounded-r-lg before:bg-primary before:content-['']",
        className,
      )}
      {...props}
    />
